...
{{--          <cite>导航元素</cite></a>--}}
    ...
    <div class="x-body">
      <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" method="get" action="/admin/admins">
          ...
          <input type="text" name="name" value="{{$request->input('name')}}" placeholder="请输入行政人员名" autocomplete="off" class="layui-input" >
          <button class="layui-btn"  lay-submit="" lay-filter="sreach"><i class="layui-icon">&#xe615;</i></button>
        </form>
      </div>
      <xblock>
        <button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon"></i>批量删除</button>
        <button class="layui-btn" onclick="x_admin_show('添加行政人员','{{url("admin/admins/create")}}',600,400)"><i class="layui-icon"></i>添加</button>
      ...
      <table class="layui-table">
        ...
        @foreach($admins as $v)
          <tr>
            ...
            <td>{{$v->id}}</td>
            <td><img src="{{asset($v->head)}}" alt=""></td>
            <td>{{$v->name}}</td>
            <td>{{$v->phone}}</td>
            <td>{{$v->education}}</td>
            <td>{{$v->duty}}</td>
